...
async function once() {
    ...
    userdbContract = await tronWeb.contract().at("TUE2vKwu6LsyN7GQvhLKnJ1S6fRNCoBWqM");
...
function getNameBalance() {
    userdbContract.nameBalance().call().then(result => {
        console.log(result)
        $('#nameBalance').text(result / 1e6)
    }).catch((err) => {
        console.log(err)
    });
}
